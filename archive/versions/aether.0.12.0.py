#!/usr/bin/env python3
"""
AETHER v0.12.0 — ONLINE LEARNED DECODER (FULL FILE)
"""
...
class HardConfig:
    ...
    NN_BATCH_SIZE = 32
    NN_TRAIN_INTERVAL = 50
    NN_SAMPLE_THRESHOLD = 0.65
...
# ============================================================================
# NEURAL DECODER (with online learning)
# ============================================================================
...
class NeuralDecoder:
    ...
    def train_on_buffer(self, epochs=None, batch_size=None, lr=None):
        ...
        print(f"[Decoder] Trained, final loss: {self.loss_history[-1]:.4f}" if self.loss_history else "")
...
class Generator:
    ...
    def generate(self, blocked):
        ...
        # (sisa kode generate wave/fractal/cellular/lsystem sama persis dengan v0.11.0)
        # ... (akan saya tulis lengkap di bawah)
        w, h = 52, 18
...
# ============================================================================
# EXTERNAL STIMULUS (sama v0.11.0)
# ============================================================================
...
# (semua sama persis dengan v0.11.0, disertakan lengkap di bawah)
# ============================================================================
...
class WorldModel:
    def __init__(self, dim=HardConfig.VECTOR_DIM, hdim=HardConfig.WORLD_MODEL_HIDDEN_DIM):
        self.W1 = np.random.randn(hdim, dim).astype(np.float32) * 0.1
        self.b1 = np.zeros(hdim, dtype=np.float32)
        self.W2 = np.random.randn(1, hdim).astype(np.float32) * 0.1
        self.b2 = 0.0
        self.confidence = 0.5
    ...
    def predict(self, vec):
        ...
        return float(np.clip(out.item(), 0, 1))

    def predict_with_uncertainty(self, vec):
        p = self.predict(vec)
        return p, 1.0 - self.confidence
...
class Aether:
    def __init__(self, workspace="aether_works_v0120", quiet=False, stimulus_source=None):
    ...
    def step(self):
        ...
        try:
            if chosen in ['generate','explore','refine','combine']:
                ...
                if chosen=='generate' and self.stimulus.is_active and self.decoder.is_trained and random.random()<0.7:
                    pred = self.decoder.predict_params(self.stimulus.vector)
                    self.generator.set_params(pred)
                ...
                if self.stimulus.is_active:
                    ...
                    if sim > HardConfig.NN_SAMPLE_THRESHOLD:
                        self.decoder.collect_sample(self.stimulus.vector, self.generator.get_params())
                        if not self.quiet: print(f"[Decoder] Collected sample, sim={sim:.3f}")
