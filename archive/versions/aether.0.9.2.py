#!/usr/bin/env python3
"""
AETHER v0.9.2 — STABILIZATION PHASE (BUGFIX)
Fixed:
- float(self.W2 @ h + self.b2) → (self.W2 @ h + self.b2).item()
- Added safe array conversion
- Ensure all vector ops return scalars
"""
...
class HardConfig:
    ...
    WORLD_MODEL_HIDDEN_DIM = 16  # reduced for stability
    VECTOR_DIM = 12  # reduced for stability
...
class ArtEmbedder:
    ...
    def embed(art: str) -> np.ndarray:
        ...
        if all_chars:
            ...
            features.append(unique/min(30, max(1,len(all_chars))))
...
class StateTransition:
    ...
    def transition(current_param_vec: np.ndarray, action: str, outcome_score: float) -> np.ndarray:
        ...
        noise = np.random.randn(len(current_param_vec)).astype(np.float32) * HardConfig.STATE_TRANSITION_NOISE
        new_vec += noise
...
if __name__ == "__main__":
    ...
    if '--auto' in sys.argv:
        ...
        print("Aether v0.9.2 — Stabilization Phase (Bugfix)")
