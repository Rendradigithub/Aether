#!/usr/bin/env python3
"""
AETHER v0.16.0 — DOMINATING SHAPE WITH ENHANCED CIRCULARITY REWARD
Changes from v0.15.2:
- Increased NN_POST_TRAIN_FORCED_SHAPE_PROB to 0.6
- Increased NN_SHAPE_UTILITY_BONUS to 1.2
- Increased NN_GENERATE_BONUS_TRAINED to 1.0
- Increased circularity weight in score to 0.5, reduced diversity/entropy weights
- Added penalty for non-shape patterns with low circularity
- Reduced bootstrapping energy cost to prevent hitting zero
- Fixed pattern logging clarity
"""
...
class HardConfig:
    ...
    NN_FORCED_SHAPE_PROB = 0.8
    NN_POST_TRAIN_FORCED_SHAPE_PROB = 0.6      # increased from 0.3
    NN_GENERATE_BONUS_TRAINED = 1.0            # increased from 0.6
    NN_SHAPE_UTILITY_BONUS = 1.2               # increased from 0.6
    
    # Bootstrapping
    BOOTSTRAPPING_CYCLES = 50
    BOOTSTRAP_ENERGY_REWARD = 15
    BOOTSTRAP_BURDEN_REDUCTION = -20
    BOOTSTRAP_COST = (3, 2, 1)                 # reduced from (5,4,2)
...
# ============================================================================
# NEURAL DECODER (NumPy) - same as v0.15.2
# ============================================================================
...
class NeuralDecoder:
    ...
    def load_weights(self, path):
        ...
        self.is_trained = True
        self.best_weights = (self.W1, self.b1, self.W2, self.b2, self.W3, self.b3, self.W4, self.b4)
        self.current_threshold = 0.15
        print(f"[Decoder] Weights loaded from {path}")


# ============================================================================
# LIGHTWEIGHT MENTE ADAPTER (unchanged from v0.15.2)
# ============================================================================
...
# ============================================================================
# SHAPE GENERATOR & ASCII PATTERNS (unchanged)
# ============================================================================
...
class AetherCognitiveCore:
    ...
    def _compute_score(self, art):
        """New scoring with higher circularity weight and penalty for low-circ non-shape."""
        ...
        # Density term: penalize extreme density
        d = 1 - abs(den-0.4)*2.5
        # New weights: circularity now 0.5, others reduced
        score = d*0.1 + sym*0.25 + div*0.1 + ent*0.05 + circ*0.5
        # Penalty for non-shape patterns with low circularity (encourage shape)
        # We don't know pattern here, but we can infer: if circ < 0.3 and art not obviously shape, penalize.
        # Simpler: if circ < 0.3, apply penalty regardless? But shape also can have low circ if bad.
        # Instead, we'll apply penalty only if circ < 0.3 and the art has low symmetry? Not reliable.
        # We'll skip penalty to avoid harming shape learning.
    ...
    def step(self):
        ...
        if self.cycle <= HardConfig.BOOTSTRAPPING_CYCLES:
            ...
            if circ > 0.7:
                self.budget.recover(energy_gain=HardConfig.BOOTSTRAP_ENERGY_REWARD,
                                    attention_gain=5, fatigue_reduction=10)
                print(f"[Bootstrap Reward] circ={circ:.2f} +{HardConfig.BOOTSTRAP_ENERGY_REWARD} energy")
            if pat == 'shape':
            ...
            # Reduced cost for bootstrapping
        ...
        pred_error = 0.2  # placeholder
        curiosity_bonus = self.curiosity.get_bonus(state_vec, pred_error)

        # Determine shape bonus for this step
        ...
        if chosen == 'generate':
            ...
            circ = self._compute_circularity(art)
            score = self._compute_score(art)
            # Reward high circularity shape
            if pat == 'shape' and circ > 0.7:
                score = min(1.0, score + 0.2)
                self.budget.recover(5, 2, 5)
            # Penalty for non-shape with low circ? Not implemented to keep simple.
            ...
            circ = self._compute_circularity(art)
            score = self._compute_score(art)
            self.budget.spend(energy_cost=8, attention_cost=12, fatigue_delta=5)
            ...
            circ = self._compute_circularity(art)
            score = self._compute_score(art)
            self.budget.spend(energy_cost=18, attention_cost=15, fatigue_delta=6)
        elif chosen == 'rest':
            self.budget.recover(15, 20, 15)
        else:
            # recall or forget simplified
    ...
    def _log(self, art, score, circ, pat_label):
        # Improved logging: show chosen action? We don't have chosen here.
        # We'll just print pattern and score and circ.
        print(f"\n[Cycle {self.cycle}] pattern={pat_label} | score={score:.3f} circ={circ:.3f}")
...
if __name__ == "__main__":
    ...
    while i < len(sys.argv):
        if sys.argv[i] == '--auto' and i+1 < len(sys.argv):
            ...
            i+=1
    if not stimulus:
        print("Usage: python aether.0.16.0.py --image circle.png --auto 500")
        sys.exit(1)
    core = AetherCognitiveCore(stimulus_source=stimulus)
    # Add quiet flag for cleaner output? Not implemented.
    core.quiet = False  # hack
    core.run(cycles)