#!/usr/bin/env python3
"""
AETHER v0.12.2 — ENHANCED SHAPE PERCEPTION & DECODER USAGE
Changes from v0.12.1:
- ArtEmbedder now includes "circularity" feature for better shape detection
- Higher forced shape probability (0.4) when decoder untrained
- If decoder is trained, generate action always uses its prediction
- Generate utility bonus when decoder trained to encourage its use
- Reset adaptive threshold on weight load for fresh sample collection
...
# ============================================================================
# HARD CONFIGURATION (updated NN_FORCED_SHAPE_PROB)
# ============================================================================
...
class HardConfig:
    ...
    NN_FORCED_SHAPE_PROB = 0.4           # increased from 0.2
    NN_GENERATE_BONUS_TRAINED = 0.5      # utility bonus when decoder trained
...
class NeuralDecoder:
    ...
    def load_weights(self, filepath: str):
        ...
        # Reset threshold so new run can collect samples easily
        self.current_threshold = HardConfig.NN_SAMPLE_THRESHOLD_INITIAL
...
# (only ArtEmbedder modified, others unchanged)
# ============================================================================
...
class ArtEmbedder:
    ...
    def embed(art):
        ...
        # NEW: Circularity / radial compactness
        cx, cy = w/2, h/2
        max_r = min(w,h)/2
        inner_count = 0
        outer_count = 0
        for y in range(h):
            for x in range(w):
                if padded[y][x] != ' ':
                    ...
                    if dist < max_r * 0.6:   # inner 60% radius
                        inner_count += 1
                    elif dist > max_r * 0.8: # outer ring
                        outer_count += 1
        # circularity = concentration in center vs outer ring, normalized
        if non > 0:
            circ = (inner_count - outer_count) / non  # ranges roughly -1..1
            circ = (circ + 1) / 2  # map to 0..1
        else:
            circ = 0.5
...
class DecisionEngine:
    ...
    def _evaluate(self, action, vec, res, blocked, stim, decoder_trained=False):
        ...
        return max(0.0, u), {}
    def _foresight(self, action, vec, res, blocked, stim, decoder_trained=False):
        u1,_ = self._evaluate(action, vec, res, blocked, stim, decoder_trained)
    ...
    def choose_action(self, feasible, res, vec, blocked, emergency, stim, decoder_trained=False):
...
class Aether:
    def __init__(self, workspace="aether_works_v0122", quiet=False, stimulus_source=None):
        ...
        self.pattern_counts = Counter()  # NEW: track pattern usage
    ...
    def step(self):
        ...
        if override and forced in feasible:
            ...
            chosen, info = self.decision.choose_action(feasible, self.resources, param_vec, blocked, emergency, self.stimulus, self.decoder.is_trained)
        ...
        try:
            if chosen in ['generate','explore','refine','combine']:
                ...
                # Shape exploration / decoder usage
                if chosen=='generate' and self.stimulus.is_active:
                    if not self.decoder.is_trained and random.random() < HardConfig.NN_FORCED_SHAPE_PROB:
                        ...
                        # Always use decoder prediction when trained
                        pred = self.decoder.predict_params(self.stimulus.vector)
                        self.generator.set_params(pred)
