#!/usr/bin/env python3
"""
AETHER v0.9.4 — STABILIZATION + BUGFIXES
Fixed:
- Shape mismatch in StateTransition (matrix broadcasting)
- Resource costs not applied (energy never decreased)
- Novelty collapse (added more generator variation)
- JSON serialization (numpy types converted to float)
- World model prediction stability
"""
...
class HardConfig:
    ...
    WORLD_MODEL_HIDDEN_DIM = 8   # reduced to avoid overfitting
    VECTOR_DIM = 6               # much smaller for stability
...
class ResourceManager:
    ...
    def apply_cost(self, action: str):
        ...
        # Death spiral multiplier (capped)
        mult = 1.0 + (self.failure_burden / HardConfig.FAILURE_BURDEN_MAX) * (HardConfig.COST_MULTIPLIER_MAX - 1)
        self.energy = self.energy + int(en * mult)
        self.attention = self.attention + int(att * mult)
        self.memory_used = self.memory_used + mem_delta
        self.fatigue = self.fatigue + fat + int(self.failure_burden // 20)
        # Clamping
...
class ArtEmbedder:
    ...
    def embed(art: str) -> np.ndarray:
        ...
        # density of non-space
        # already done, but just add another stat: variation of run lengths
        # just pad to dimension
        while len(features) < HardConfig.VECTOR_DIM:
            features.append(0.0)
        vec = np.array(features[:HardConfig.VECTOR_DIM], dtype=np.float32)
        norm = np.linalg.norm(vec)
...
# ============================================================================
# STATE TRANSITION (fixed broadcasting)
# ============================================================================
...
# ============================================================================
# GENERATOR (more variation to avoid novelty collapse)
# ============================================================================
...
class Generator:
    ...
    def generate(self, blocked_patterns: Set[str]) -> Tuple[str, str]:
        ...
        # Symmetry (apply at probability = symmetry param)
        if random.random() < self.params['symmetry']:
            for y in range(h):
                for x in range(w//2):
                    if grid[y][x] != ' ':
...
# ============================================================================
# DECISION ENGINE (simplified to avoid errors)
# ============================================================================
    ...
    def step(self) -> Dict:
        ...
        try:
            if chosen_action in ['generate', 'explore', 'refine', 'combine']:
                ...
                # Score from art features (heuristic)
...
if __name__ == "__main__":
    ...
    if '--auto' in sys.argv:
        ...
        print("Aether v0.9.4 — Stabilization Phase (bug fixes)")
