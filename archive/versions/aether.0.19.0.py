#!/usr/bin/env python3
"""
AETHER v0.19.0 — BEHAVIORAL MOMENTUM & STATE PREDICTION
================================================================
Changes from v0.18.0:
- Adaptive reset now requires: (repeat >= 5) AND (no reward improvement over 8 cycles)
- Added area coherence reward (connected component ratio) to kill reward hacking
- World model now predicts next state (radial signature) instead of reward
- Curiosity bonus = prediction error of state (uncertainty-driven exploration)
- Forced shape probability: 0.65 (adaptive: higher if contour reward low)
- Ideal contour generation improved (binary edge without noise)
- Reduced rest frequency by increasing rest cooldown to 5 cycles
- Added persistence bonus: if reward > 0.7, avoid reset for 5 cycles
"""
...
class HardConfig:
    ...
    WORLD_MODEL_HIDDEN_DIM = 16          # increased for state prediction
    VECTOR_DIM = 36                      # radial signature length
    ...
    NN_POST_TRAIN_FORCED_SHAPE_PROB = 0.65      # increased from 0.5 for stability
    ...
    CURIOSITY_BONUS = 0.4                     # increased for state prediction error
    ...
    REST_COOLDOWN = 5                          # increased from 3
    ...
    # Stagnation thresholds (more conservative)
    REPETITION_STAGNATION_THRESHOLD = 5        # was 10, but combined with reward window
    REWARD_STAGNATION_WINDOW = 8
    REWARD_IMPROVEMENT_THRESHOLD = 0.03
    PERSISTENCE_BONUS_CYCLES = 5              # if reward > 0.7, delay reset
    ...
    REWARD_SURVIVAL_WEIGHT = 0.0             # moved to separate survival bonus


# ============================================================================
# NEURAL DECODER (unchanged from v0.18.0)
# ============================================================================
...
# ============================================================================
# RADIAL SIGNATURE & CONTOUR UTILITIES (v0.19.0)
# ============================================================================
...
class RadialSignature:
    ...
    def from_ascii_art(art, num_rays=36, contour_only=True):
        """Extract radial signature from ASCII art."""
        lines = [l.rstrip('\n') for l in art.split('\n') if l.strip()]
        if not lines:
    ...
    def ideal_contour_from_params(shape_param, symmetry, density, noise, w=52, h=18, num_rays=36):
        """Generate idealized binary edge map (no filler, no noise)."""
        # Use shape generator but force density low and noise 0
        ...
        # Convert to binary edge (thick edge already)
...
# ============================================================================
# AREA COHERENCE UTILITY (NEW)
# ============================================================================

class AreaCoherence:
    @staticmethod
    def largest_connected_component_ratio(art):
        """Return ratio of largest 4-connected component to total non-space cells."""
        lines = [l.rstrip('\n') for l in art.split('\n') if l.strip()]
        if not lines:
            return 0.0
        h, w = len(lines), max(len(l) for l in lines)
...
# ============================================================================
# PREDICTIVE WORLD MODEL (STATE PREDICTION)
# ============================================================================

class PredictiveWorldModel:
    def __init__(self, input_dim=HardConfig.VECTOR_DIM, hidden_dim=HardConfig.WORLD_MODEL_HIDDEN_DIM):
        # Predict next state (radial signature) from current state + action
        self.W1 = np.random.randn(hidden_dim, input_dim + 1) * 0.1  # +1 for action index
    ...
    def update(self, state_vec, action, next_state_vec):
        ...
        # Simple linear update (approximate gradient)
        x = np.append(state_vec, self.encode_action(action))
        h = np.tanh(self.W1 @ x + self.b1)
...
# ============================================================================
# IMPROVED SHAPE GENERATOR (clean contour + area coherence friendly)
# ============================================================================
...
class ShapeAwareGenerator:
    ...
    def generate_shape(shape_param, symmetry, density, noise, w=52, h=18):
        ...
        # Filler inside (for area coherence)
        if density > 0.3:
            if shape_param < 0.33:
                r_inner = max(1, r-2)
                for y in range(h):
...
# ============================================================================
# LIGHTWEIGHT MENTE ADAPTER (v0.19.0)
# ============================================================================
...
class MenteBudget:
    def __init__(self, max_energy=100, max_attention=100):
        ...
        self.persistence_counter = 0   # new: avoid reset when high reward
    ...
    def should_reset(self, reward):
        # New logic: reset only if pattern repeated >=5 AND reward stagnant over window AND no persistence
...
class MenteCuriosity:
    ...
    def get_bonus(self, state_vec, action):
        # Use state prediction error as curiosity
...
class AetherCognitiveCore:
    def __init__(self, stimulus_source=None, workspace="aether_works_v0190", quiet=False):
    ...
    def _compute_reward(self, art, pattern):
        ...
        novelty = self.memory.novelty(avec) if hasattr(self.memory, 'novelty') else 0.5
        # Total reward
        ...
        # Survival bonus (optional, not included in main weighting)
        # survival = (self.budget.energy/self.budget.max_energy + self.budget.attention/self.budget.max_attention + (1-self.budget.fatigue/HardConfig.MAX_FATIGUE))/3
        # total = total * 0.9 + survival * 0.1   # small survival influence
        total = min(1.0, max(0.0, total))
        return total, radial_sim, contour_reward, coherence

    def _detect_stagnation(self, pattern, reward):
        # New stagnation detection uses budget.should_reset
    ...
    def step(self):
        ...
        if self.bootstrapping_phase and self.cycle <= self.bootstrapping_end_cycle:
            ...
            # Update world model with state transition
            # For bootstrap, next state is same as current? Actually art changes. We'll use current radial as next.
        ...
        # Determine shape bonus for generate (adaptive forced shape)
        shape_bonus = False
        if 'generate' in feasible:
            # Adaptive forced shape: if recent contour reward low, increase probability
        ...
        if chosen == 'generate':
            ...
            # Update world model (state prediction)
            next_sig = RadialSignature.from_ascii_art(art, num_rays=36, contour_only=True)
            self.world_model.update(self.stimulus_radial, chosen, next_sig)
            # Memory
        ...
        # Update reward history for stagnation detection
...
if __name__ == "__main__":
    ...
    while i < len(sys.argv):
        if sys.argv[i] == '--auto' and i+1 < len(sys.argv):
            ...
            i+=1
    if not stimulus:
        print("Usage: python aether.0.19.0.py --image circle.png --auto 500")
        sys.exit(1)
