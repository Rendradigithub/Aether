#!/usr/bin/env python3
# =============================================================================
# AETHER v0.3: Adaptive Extensible Heuristic Engine with Training & Evolution
# =============================================================================
# UPGRADES from v0.2:
#   - ArtEngine: Aether bisa "melukis" ASCII art di sandbox/
#   - Ekspresi otomatis setelah deploy skill baru (celebrate)
#   - Ekspresi rutin setiap 5 cycle autonomous
#   - Tema dipilih Aether sendiri berdasarkan konteks internal
#   - Art history: tidak mengulang tema berturut-turut
#   - express() tersedia sebagai perintah manual
#
# UPGRADES from v0.1:
#   - Autonomous Loop (background thread, non-blocking)
#   - GoalEngine: goal generator yang belajar dari knowledge base sendiri
#   - ReflectionLayer: evaluasi diri + momentum tracking per siklus
#   - Hybrid mode: manual + autonomous berjalan bersamaan
#   - FIX: bool ditambahkan ke sandbox restricted_builtins
#
# CORE PRINCIPLES:
...
class ArtEngine:
    ...
    def _choose_tema(self, context: Dict) -> Dict:
        """
        Pilih tema berdasarkan konteks.
        Tidak mengulang tema yang sama dua kali berturut-turut.
        """
        skill      = context.get('skill', '')
        state      = context.get('state', 'normal')        # momentum/stagnant/normal
        trigger    = context.get('trigger', 'routine')     # celebrate/routine
        category   = context.get('category', '')

        # Bangun tag kandidat dari konteks
        want_tags = set()
        want_tags.add(trigger)   # 'celebrate' atau 'routine'
        want_tags.add(state)     # 'momentum', 'stagnant', 'normal'
        if skill:    want_tags.add(skill)
        if category: want_tags.add(category)

        # Score tiap entry berdasarkan kecocokan tag
        scored = []
        last_tema = self._art_history[-1] if self._art_history else None

        for entry in self._catalog:
            if entry['tema'] == last_tema:
                continue   # jangan ulang tema terakhir
            match = len(want_tags & set(entry['tags']))
            scored.append((match, entry))

        if not scored:
            # fallback: semua kecuali yang terakhir
            scored = [(0, e) for e in self._catalog if e['tema'] != last_tema]
        
        # Pilih secara probabilistik, lebih tinggi score = lebih sering dipilih
        max_score = max(s for s, _ in scored)
        top = [(s, e) for s, e in scored if s == max_score]
    ...
    def _run(self):
        ...
        while not self._stop_event.is_set():
            ...
            try:
                ...
                # 6b. NEW v0.3: ekspresi rutin tiap 5 cycle
                if self._cycle_count % 5 == 0:
                    ...
                    self.aether.art_engine.express({
                        ...
                        'consecutive_failures': self.aether.reflection.consecutive_failures,
    ...
    def improve(self, task_description: str) -> Tuple[bool, str]:
        ...
        if best_code and best_score > 0.0:
            ...
            if success:
                ...
                # NEW v0.3: celebrate dengan ASCII art
                ...
                self.art_engine.express({
                    ...
                    'state':          'momentum',
                    'mastered_count': len(mastered),
                    'cycle':          getattr(self._autonomous_loop, '_cycle_count', 'manual'),
                })
