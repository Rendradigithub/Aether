#!/usr/bin/env python3
"""
AETHER v0.9.11 — EXTERNAL STIMULUS INTEGRATION
"Now I can see the world outside my loop. It influences my decisions."

NEW IN v0.9.11:
- External stimulus slot: Aether can receive an external vector (e.g., from images)
- Modified utility function: similarity between current state and stimulus adds a bonus
- Goal engine can be biased by external preference
- Simple JSON input for stimulus (to test integration)
- Full backwards compatibility with v0.9.10

PURPOSE:
  This is the foundation BEFORE adding visual perception.
  Aether now reacts to external input, proving the integration works.

USAGE:
  python aether_0_9_11.py --auto 200                # run with no stimulus (default)
  python aether_0_9_11.py --auto 200 --stimulus vector.json   # run with external stimulus
  python aether_0_9_11.py --demo                    # quick demo

STRUCTURE:
  - ExternalStimulus class: loads and manages stimulus vector
  - Modified DecisionEngine: adds stimulus bonus to utility
  - Modified GoalEngine: can incorporate stimulus preference
  - Unchanged ResourceManager, LongTermScars, Generator, etc.

NOTE: This version does NOT include image processing yet.
      Only the integration layer for future visual/audio input.
"""
...
# ============================================================================
# HARD CONFIGURATION (unchanged from v0.9.10)
# ============================================================================
...
class HardConfig:
    ...
    # NEW: Stimulus integration parameters
    STIMULUS_WEIGHT = 0.25          # how much stimulus influences utility (0-1)
    STIMULUS_SIMILARITY_THRESHOLD = 0.1   # minimum similarity to have effect


# ============================================================================
# EXTERNAL STIMULUS (new)
# ============================================================================

class ExternalStimulus:
    """
    Represents input from outside (future: images, audio, sensor data).
    Currently loads a vector from JSON file.
    
    The vector should be in the same embedding space as ParamEmbedder.
    Memory will store and recall stimulus-response pairs.
    """
    
    def __init__(self, stimulus_file: Optional[str] = None):
        self.vector = None          # type: np.ndarray
        self.metadata = {}          # original stimulus info (e.g., source, type)
        self.is_active = False
        
        if stimulus_file:
            self.load(stimulus_file)
    
    def load(self, filepath: str) -> bool:
        """Load stimulus from JSON file."""
        try:
            with open(filepath, 'r') as f:
                data = json.load(f)
            
            # Expected format: {"vector": [0.1, 0.2, ...], "source": "image", "timestamp": "...", "description": "..."}
            if 'vector' in data:
                ...
                self.is_active = True
                print(f"[Stimulus] Loaded from {filepath} | dim={len(vec)} | source={self.metadata.get('source', 'unknown')}")
                return True
            else:
    ...
    def set_vector(self, vector: np.ndarray, metadata: Dict = None):
        """Manually set stimulus vector (e.g., from image processing in future)."""
        # Ensure normalized
        ...
        self.is_active = True
        print(f"[Stimulus] Vector set manually | dim={len(vector)}")
    ...
    def similarity(self, other_vec: np.ndarray) -> float:
        """Compute cosine similarity between stimulus and other vector."""
...
# ============================================================================
# DECISION ENGINE (modified: adds stimulus bonus)
# ============================================================================
...
# ============================================================================
# MAIN AETHER (with external stimulus)
# ============================================================================

class Aether:
    def __init__(self, workspace="aether_works_v0911", quiet=False, stimulus_file=None):
        ...
        self.stimulus = ExternalStimulus(stimulus_file)   # NEW
...
# ============================================================================
# UTILITY: generate sample stimulus JSON
# ============================================================================

def generate_sample_stimulus():
    """Generate a sample stimulus JSON file for testing."""
    # Example: prefer high symmetry and medium density
...
if __name__ == "__main__":
    ...
    for i, arg in enumerate(sys.argv):
        if arg == '--stimulus' and i+1 < len(sys.argv):
            stimulus_file = sys.argv[i+1]
    ...
    if '--auto' in sys.argv or '--demo' in sys.argv:
        a = Aether(quiet=False, stimulus_file=stimulus_file)
        a.run_autonomous(cycles)
    else:
        print("Aether v0.9.11 — External Stimulus Integration")
        print("Run with:")
        print("  python aether_0_9_11.py --auto 200")
        print("  python aether_0_9_11.py --auto 200 --stimulus stimulus.json")
        print("  python aether_0_9_11.py --generate-sample   # create sample stimulus")
        a = Aether(quiet=True, stimulus_file=stimulus_file)
        a.run_autonomous(10)