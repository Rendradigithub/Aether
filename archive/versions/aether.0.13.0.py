#!/usr/bin/env python3
"""
AETHER v0.13.0 — FIXED SHAPE LEARNING FROM STIMULUS
Changes from v0.12.2:
- NN_FORCED_SHAPE_PROB raised to 0.8 for intensive shape exploration.
- After decoder trained, still 30% chance to force shape to prevent biased locking.
- Only collect training samples when generator produces 'shape' pattern.
- Added utility bonus (+0.6) for generate actions that produce 'shape'.
- Reward shaping: if shape has high circularity (>0.7), increase score directly.
- Reset threshold on weight load for fresh collection.
"""
...
class HardConfig:
    ...
    NN_FORCED_SHAPE_PROB = 0.8               # increased from 0.4
    NN_POST_TRAIN_FORCED_SHAPE_PROB = 0.3   # even after trained, keep exploring shapes
    NN_GENERATE_BONUS_TRAINED = 0.6         # utility bonus when decoder trained
    NN_SHAPE_UTILITY_BONUS = 0.6            # extra bonus for generating 'shape' pattern
...
# ============================================================================
# NEURAL DECODER (unchanged from v0.12.2, but training only on shape samples)
# ============================================================================
...
class NeuralDecoder:
    ...
    def collect_sample(self, stimulus_vec: np.ndarray, generator_params: Dict):
        # ONLY collect if pattern is 'shape' to avoid polluting buffer with wave/fractal/etc.
...
# (only DecisionEngine._evaluate modified to add shape bonus, others unchanged)
# ============================================================================
...
class ArtEmbedder:
    ...
    def embed(art):
        ...
        # Circularity / radial compactness
        cx, cy = w/2, h/2
        max_r = min(w,h)/2
        inner_count = 0
        outer_count = 0
            ...
            for x in range(w):
                if padded[y][x] != ' ':
                    ...
                    if dist < max_r * 0.6:
                        ...
                        outer_count += 1
        if non > 0:
            circ = (inner_count - outer_count) / non
            circ = (circ + 1) / 2
        else:
            circ = 0.5
...
class DecisionEngine:
    ...
    def _evaluate(self, action, vec, res, blocked, stim, decoder_trained=False, shape_bonus=False):
        ...
        # Bonus for generate when decoder is trained
        if action == 'generate' and decoder_trained:
            u += HardConfig.NN_GENERATE_BONUS_TRAINED
        # Extra bonus if this action will produce shape (passed as flag)
        if action == 'generate' and shape_bonus:
            u += HardConfig.NN_SHAPE_UTILITY_BONUS
        return max(0.0, u), {}
    def _foresight(self, action, vec, res, blocked, stim, decoder_trained=False, shape_bonus=False):
        u1,_ = self._evaluate(action, vec, res, blocked, stim, decoder_trained, shape_bonus)
        ...
        best = max((self._evaluate(a2, next_vec, sim, blocked, stim, decoder_trained)[0] for a2 in HardConfig.ACTION_COSTS if a2 not in blocked), default=0.0)
        return u1 + HardConfig.PLANNING_DISCOUNT*best
    def choose_action(self, feasible, res, vec, blocked, emergency, stim, decoder_trained=False, shape_bonus=False):
        ...
        for a in feasible:
            if res.energy>20 and a in ['generate','explore','refine','combine']:
                # for generate, if shape_bonus is True, we give foresight with bonus
                if a=='generate' and shape_bonus:
                    utils[a] = self._foresight(a, vec, res, blocked, stim, decoder_trained, shape_bonus=True)
                else:
                    utils[a] = self._foresight(a, vec, res, blocked, stim, decoder_trained)
            else: utils[a],_ = self._evaluate(a, vec, res, blocked, stim, decoder_trained)
...
class Aether:
    def __init__(self, workspace="aether_works_v0122", quiet=False, stimulus_source=None):
    ...
    def step(self):
        ...
        # Determine if we will force shape for this generate action
        force_shape = False
        shape_bonus_for_action = False
        if 'generate' in feasible:
            # Before training: forced shape with high probability
            if not self.decoder.is_trained:
                if random.random() < HardConfig.NN_FORCED_SHAPE_PROB:
                    force_shape = True
                    shape_bonus_for_action = True
            else:
                # After training: still sometimes force shape to keep exploring
        ...
        if override and forced in feasible:
            ...
            chosen, info = self.decision.choose_action(feasible, self.resources, param_vec, blocked, emergency, self.stimulus, self.decoder.is_trained, shape_bonus=shape_bonus_for_action)
            ...
            if chosen in ['generate','explore','refine','combine']:
                ...
                if chosen=='generate':
                    if force_shape:
                        ...
                        # bias toward circle (shape_param < 0.33)
                        self.generator.params['shape_param'] = random.uniform(0, 0.4)
                        if not self.quiet: print("[Forced Shape] Exploring geometric shape (circle biased)")
                    elif self.decoder.is_trained:
                        # Use decoder prediction normally
                        pred = self.decoder.predict_params(self.stimulus.vector)
                        self.generator.set_params(pred)
                    # else: no forced shape, keep current params (may be whatever)
                ...
                # Reward shaping: if pattern is shape and circularity > 0.7, increase score
                if pat == 'shape':
                    circ = feats.get('circularity', 0.5)
                    if circ > 0.7:
                        score = min(1.0, score + 0.2)
                        if not self.quiet: print(f"[Reward] High circularity {circ:.2f}, score boosted to {score:.3f}")
                ...
                if self.stimulus.is_active:
                    ...
                    # Only collect sample if pattern is shape (already enforced inside collect_sample)
                    self.decoder.collect_sample(self.stimulus.vector, self.generator.get_params())
                    if sim > current_thresh and not self.quiet:
                        print(f"[Decoder] Collected sample, sim={sim:.3f} (thresh={current_thresh:.2f}, buffer={len(self.decoder.training_buffer)})")
        ...
        if not self.quiet:
            print(f"\n[Cycle {self.cycle}] {chosen} | score={score:.3f} nov={nov:.3f} pattern={pat if art else 'N/A'}")
    ...
    def _extract_features(self, art):
        ...
        if not lines: return {'symmetry':0,'density':0,'diversity':0,'entropy':0, 'circularity':0}
        ...
        # Compute circularity same as ArtEmbedder
        cx, cy = w/2, h/2
        max_r = min(w,h)/2
        inner_count = 0
        outer_count = 0
        ...
        if non > 0:
            ...
            circ = 0.5
        return {'symmetry':sym,'density':den,'diversity':div,'entropy':ent, 'circularity':circ}
    
    def _compute_score(self, f):
        d = 1 - abs(f['density']-0.4)*2.5
        # circularity contributes 20% to score
        return max(0, min(1, d*0.2 + f['symmetry']*0.3 + f['diversity']*0.2 + f['entropy']*0.1 + f['circularity']*0.2))
    ...
    def run_autonomous(self, cycles=200, save_log=True):
        for _ in range(cycles):
            self.step()
            time.sleep(0.1)  # sedikit lebih cepat
        print("\n=== RUN SUMMARY ===")
        print(f"Total cycles: {cycles}")
        print(f"Pattern usage: {dict(self.pattern_counts)}")
        if self.decoder.loss_history:
            print(f"Final decoder loss: {self.decoder.loss_history[-1]:.4f}, best: {self.decoder.best_loss:.4f}")
        if save_log:
