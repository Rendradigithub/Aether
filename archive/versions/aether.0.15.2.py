#!/usr/bin/env python3
"""
AETHER v0.15.2 — COGNITIVE ARCHITECTURE (NumPy Only, Stable)
- Menggunakan NeuralDecoder berbasis NumPy (tanpa TinyGrad)
- Bootstrapping 50 siklus shape ideal
- Mente: tiered memory, budget, curiosity, event bus
"""
...
# ============================================================================
# NEURAL DECODER (NumPy) - sama seperti v0.13.0
# ============================================================================
class NeuralDecoder:
...
# ============================================================================
# SHAPE GENERATOR & EMBEDDER (sama seperti sebelumnya)
# ============================================================================
...
class AetherCognitiveCore:
    ...
    def step(self):
        ...
        curiosity_bonus = self.curiosity.get_bonus(self.stimulus_vector, pred_error)
        utils = {}
        for a in feasible:
            base = 0.5
            if a == 'generate':
        ...
        if chosen == 'generate':
            ...
            self.memory.add_experience({'state': self.stimulus_vector.tolist(), 'action': chosen,
                                        'reward': score, 'pattern': pat, 'circularity': circ,
...
if __name__ == "__main__":
    ...
    while i < len(sys.argv):
        if sys.argv[i] == '--auto' and i+1 < len(sys.argv):
            ...
            i+=1
    if not stimulus:
        print("Usage: python aether.0.15.2.py --image circle.png --auto 500")
        sys.exit(1)
